...
import { useMe, useSessions, useRevokeSession, useChangePassword, useLogoutAll } from '@/hooks/use-auth'
...
function SettingsPage() {
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-(--gray-12) mb-1.5">Pengaturan</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <p className="text-sm font-medium text-(--gray-12)">{user.email || '-'}</p>
            {user.email && (
              <Badge variant={user.email_verified ? 'success' : 'warning'} className="text-[10px]">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <p className="text-sm font-medium text-(--gray-12)">{user.whatsapp || '-'}</p>
            {user.whatsapp && (
              <Badge variant={user.whatsapp_verified ? 'success' : 'warning'} className="text-[10px]">
  ...
  return (
    <div className="rounded-2xl border border-(--gray-4) bg-(--color-surface) p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-sm font-semibold text-(--gray-12)">Ubah Password</h2>
      </div>

      <form onSubmit={onSubmit} noValidate className="flex flex-col gap-4 max-w-md">
        {/* Old Password */}
        <div className="flex flex-col gap-1.5">
          <label htmlFor="old-password" className="text-sm font-medium text-(--gray-12)">Password Lama</label>
          <div className="relative">
            <input id="old-password" type={showOld ? 'text' : 'password'} placeholder="••••••••"
              {...register('old_password')} className={cn(inputCls(!!errors.old_password), 'pr-10')} />
            <button type="button" tabIndex={-1} onClick={() => setShowOld(v => !v)}
              ...
              {showOld ? <EyeClosedIcon className="h-4 w-4" /> : <EyeOpenIcon className="h-4 w-4" />}
            </button>
          </div>
        ...
        {/* New Password */}
        <div className="flex flex-col gap-1.5">
          <label htmlFor="new-password" className="text-sm font-medium text-(--gray-12)">Password Baru</label>
          <div className="relative">
            <input id="new-password" type={showNew ? 'text' : 'password'} placeholder="••••••••"
              {...register('new_password')} className={cn(inputCls(!!errors.new_password), 'pr-10')} />
            <button type="button" tabIndex={-1} onClick={() => setShowNew(v => !v)}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-(--gray-9) hover:text-(--gray-12) transition-colors cursor-pointer">
              {showNew ? <EyeClosedIcon className="h-4 w-4" /> : <EyeOpenIcon className="h-4 w-4" />}
            </button>
          </div>
        ...
        {/* Confirm Password */}
        <div className="flex flex-col gap-1.5">
          <label htmlFor="confirm-password" className="text-sm font-medium text-(--gray-12)">Konfirmasi Password Baru</label>
          <input id="confirm-password" type="password" placeholder="••••••••"
            {...register('confirm_password')} className={inputCls(!!errors.confirm_password)} />
          {errors.confirm_password && <p className="text-xs text-red-500">{errors.confirm_password.message}</p>}
        </div>

        <Button type="submit" variant="solid" size="2" loading={changePasswordMutation.isPending} className="w-fit mt-2">
  ...
  return (
    <div className="rounded-2xl border border-(--gray-4) bg-(--color-surface) p-6">
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-(--gray-9) text-center py-8">Tidak ada sesi aktif.</p>
      ) : (
          ...
          {sessions.map((session) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-(--gray-12)">{session.device_name || 'Perangkat Tidak Dikenal'}</p>
                  <p className="text-xs text-(--gray-9)">
                    {session.ip_address}
                    {' · '}
                    Terakhir aktif: {new Date(session.last_used_at).toLocaleDateString('id-ID', {
