...
const verifyOtpSchema = z.object({
  identifier: z.string().regex(/^\+62\d{9,13}$/, 'Format nomor WhatsApp tidak valid. Harus diawali +62.'),
    ...
    <div className="min-h-screen flex items-center justify-center bg-(--color-background) px-6 py-12">
      <div className="w-full max-w-[400px] rounded-2xl border border-(--gray-5) bg-(--color-surface) p-8 shadow-sm">
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-(--gray-12) mb-1.5">Verifikasi WhatsApp</h2>
          <p className="text-sm text-(--gray-10)">
            Masukkan 6 digit kode OTP yang dikirimkan ke nomor WhatsApp Anda untuk mengaktifkan akun.
          </p>
        </div>
        ...
        <form onSubmit={onSubmit} noValidate className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="verify-whatsapp-input" className="text-sm font-medium text-(--gray-12)">Nomor WhatsApp</label>
            <input id="verify-whatsapp-input" type="tel" placeholder="+628123456789"
