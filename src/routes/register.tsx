...
const registerSchema = z.object({
  ...
  whatsapp: z.string().regex(/^\+62\d{9,13}$/, 'Format nomor WhatsApp tidak valid. Harus diawali +62.').optional(),
...
function RegisterPage() {
  ...
  const {
    ...
    defaultValues: { name: '', method: 'email', email: '', whatsapp: '', password: '' },
  })
  ...
  const onSubmit = handleSubmit(async (data) => {
    const payload = {
      ...
      ...(data.method === 'email' ? { email: data.email } : { whatsapp: data.whatsapp }),
    }
    ...
    await registerMutation.mutateAsync(payload, {
      onSuccess: () => {
        ...
        if (data.method === 'email') {
          ...
          navigate({ to: '/verify-whatsapp', search: { identifier: data.whatsapp } })
        }
      },
    })
  })
    ...
    <div className="min-h-screen flex bg-(--color-background)">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 sm:px-12">
        ...
        <div className="w-full max-w-[400px]">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-(--gray-12) mb-1.5">Buat Akun Baru</h2>
          ...
          <form onSubmit={onSubmit} noValidate className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="reg-name" className="text-sm font-medium text-(--gray-12)">Nama Lengkap</label>
              <input id="reg-name" type="text" placeholder="Jiilan Nashrulloh"
                {...regForm('name')} className={inputCls(!!errors.name)} />
            ...
            {method === 'email' && (
              <div className="flex flex-col gap-1.5">
                <label htmlFor="reg-email" className="text-sm font-medium text-(--gray-12)">Alamat Email</label>
                <input id="reg-email" type="email" placeholder="nama@email.com"
                  {...regForm('email')} className={inputCls(!!errors.email)} />
            ...
            {method === 'whatsapp' && (
              <div className="flex flex-col gap-1.5">
                <label htmlFor="reg-whatsapp" className="text-sm font-medium text-(--gray-12)">Nomor WhatsApp</label>
                <input id="reg-whatsapp" type="tel" placeholder="+628123456789"
                  {...regForm('whatsapp')} className={inputCls(!!errors.whatsapp)} />
            ...
            {/* Password */}
            <div className="flex flex-col gap-1.5">
              <label htmlFor="reg-password" className="text-sm font-medium text-(--gray-12)">Password</label>
            ...
            <Button type="submit" variant="solid" size="3" loading={registerMutation.isPending} className="w-full mt-1"
              rightIcon={!registerMutation.isPending ? <ArrowRightIcon /> : undefined}>
