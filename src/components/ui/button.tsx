...
const buttonVariants = cva(
  [
    ...
    "focus-visible:ring-(--blue-9) focus-visible:ring-offset-background",
  ...
  ],
  {
    variants: {
      variant: {
        solid: ["bg-(--blue-9) text-(--blue-contrast)", "hover:bg-(--blue-10)"],
        soft: ["bg-(--blue-a3) text-(--blue-11)", "hover:bg-(--blue-a4)"],
        outline: [
          "border border-(--blue-7) text-(--blue-11) bg-transparent",
          "hover:bg-(--blue-a3)",
        ],
        ghost: ["text-(--gray-12) bg-transparent", "hover:bg-(--gray-a3)"],
        surface: [
  ...
  (
    ...
    return (
      <button
        ...
        {...props}
      >
        {loading ? <UpdateIcon className="h-4 w-4 animate-spin" /> : leftIcon}
        {children}
        {!loading && rightIcon}
      </button>
    );
